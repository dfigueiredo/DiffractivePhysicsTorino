...
void Efficiency(){
  ...
  for (Nloop = 1; Nloop<=LumBin; Nloop++ ){
	...
	for (p=0;p<MaxVtxInEvent;p++)
	  {
	    ...
	    pois_theory[Nloop-1]->Fill(p+0.01,prob_evt(Sigma,LumiR,p));
	...
	for (Int_t i=1; i<=Draw;i++)
	{
	  ...
	  // cout << "Event "<< i << " start: "<< ka << " =  Number of Vertexes in addition to the primary trigger" << endl;
		...
		if (HadronF)    //decides if it's detected
		  {
			...
			vtx_gen_mult[NVtxAfterMultEff] = Vertex1Mult->GetRandom(); // PU multiplicity for merging
		      }
	...
	for (p = 1;p<=NVtxAfterMultEff;p++)
	  {
	    ...
	    if (DAV && Dist > DavDist) // DavDist =0.06; = 0.1 for SystCheck1
...
===============
	...
	Int_t FakeVtx = 0;
	for (Int_t NC = 1;NC<=counter;NC++)
	  {
	    if ( r.Uniform(0.,1.) < FracSecVertex)  FakeVtx++;
	  }
	counter = counter + FakeVtx; // Add fake seconday Vtx
	EffCounter  += FakeVtx;
	...
	NHSetMarker(pois_meas[Nloop-1],2,20,0.8);
	pois_meas[Nloop-1]->Draw();
	pois_theory[Nloop-1]->Draw("SAMEHIST");    
	pois_meas[Nloop-1]->GetXaxis()->SetTitle("vertex");
	pois_meas[Nloop-1]->GetYaxis()->SetTitle("Fraction");
	...
	pois_ratio[Nloop-1]->GetXaxis()->SetTitle("Vertex");
	pois_ratio[Nloop-1]->GetYaxis()->SetTitle("Predicted Vis./True");
...
void CheckVxt(){
  ...
  for (Int_t N=1;N<14; N++){
    ...
    VxtLumiBin[N]->SetMinimum(0.0001);
...
void MCReweightGT2(){
  ...
  TH1F *PtMeas =   (TH1F*)fData->Get("MeasPt_NDOFGT2_Mult9");

  if (Vertex1Mult == 0 || PtMeas == 0)
    {
  ...
  stringa = "TracksMeas_NDOFGT2_Z2";
  sprintf(hname,stringa.c_str());
  TH1F *VertexMultZ2 =    (TH1F*)fMC->Get(hname);
  stringa = "TracksMeas_NDOFGT2_w_Z2";
  sprintf(hname,stringa.c_str());
  TH1F *VertexMultZ2w =    (TH1F*)fMC->Get(hname)
;
  stringa = "MeasPt_NDOFGT2_Mult9_Z2";
  sprintf(hname,stringa.c_str());
  TH1F *PtZ2 =    (TH1F*)fMC->Get(hname);

  stringa = "TracksMeas_NDOFGT2_w_CW";
  sprintf(hname,stringa.c_str());
  TH1F *VertexMultCWw =    (TH1F*)fMC->Get(hname);
  stringa = "TracksMeas_NDOFGT2_CW";
  sprintf(hname,stringa.c_str());
  TH1F *VertexMultCW =    (TH1F*)fMC->Get(hname);

  stringa = "MeasPt_NDOFGT2_Mult9_CW";
  sprintf(hname,stringa.c_str());
  ...
  //  TH1F *VertexMultD6T =    (TH1F*)fD6T->Get("TracksMeas_NDOFGT2_w");
  //TH1F *VertexMultZ2 =    (TH1F*)fZ2->Get("TracksMeas_NDOFGT2_w");


  NH1FArea1(Vertex1Mult);
  NH1FArea1(VertexMultCW);
  NH1FArea1(VertexMultZ2);

  NH1FArea1(VertexMultCWw);
  NH1FArea1(VertexMultZ2w);
  ...
  Rew->cd(1);
  NLogAxis(1,1);
  Vertex1Mult->SetAxisRange(3,120);
  ...
  VertexMultZ2w->SetLineColor(4);
  VertexMultZ2w->Draw("SAMEHIST");
  ...
  leg->AddEntry(VertexMultZ2w,"Z2","L");  
  leg->SetBorderSize(0);
  //  leg->SetTextSize(0.1);  
  leg->Draw();


  TCanvas *My = NCanvas(1,2,"My");
  ...
  //  leg->SetTextSize(0.1);  
  leg->Draw();
  TH1F *ratioz2 = NH1clone("Data/Z2_NDOFGT2","Data/Z2_NDOFGT2",Vertex1Mult);
  TH1F *ratiocw = NH1clone("Data/CW_NDOFGT2","Data/CW_NDOFGT2",Vertex1Mult);
  ...
  ratioz2->Divide(VertexMultZ2);
  //  NH1FMax1(ratioz2);
  //  ratioz2->SetMaximum(5);
  // ratioz2->SetMinimum(0.);
  ratioz2->Draw();
  ratioz2->Fit("pol3","","",4,100);
