...
// Mon Feb  7 08:18:04 2011 by ROOT version 5.18/00
// from TTree tree_/tree_
// found on file: files/ZetaSkim_ee_v1.root
//////////////////////////////////////////////////////////

#ifndef Reweight_Class_h
#define Reweight_Class_h
...
public :
 ...
 //vector<double>  vertexMolteplicity;
   ...
   Reweight_Class(TTree *tree=0);
   virtual ~Reweight_Class();
...
#ifdef Reweight_Class_cxx
Reweight_Class::Reweight_Class(TTree *tree)
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("files/ZetaSkim_ee_v1.root");
      if (!f) {
         f = new TFile("files/ZetaSkim_ee_v1.root");
      }
...
Reweight_Class::~Reweight_Class()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t Reweight_Class::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t Reweight_Class::LoadTree(Long64_t entry)
{
...
void Reweight_Class::Init(TTree *tree)
{
...
void Reweight_Class::Show(Long64_t entry)
{
...
Int_t Reweight_Class::Cut(Long64_t entry)
{
...
#endif // #ifdef Reweight_Class_cxx
