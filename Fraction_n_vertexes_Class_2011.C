#define Fraction_n_vertexes_Class_cxx
#include "Fraction_n_vertexes_Class_2011.h"
#include <TH2.h>
#include <TH2F.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void Fraction_n_vertexes_Class_2011::Loop()
{
  ...
  Dat_all_Lumi = new TH1F("Dat_all_Lumi", "Dat_all_Lumi", LumBin, 0., 1.4);
  Dat_all_Lumi->Sumw2();

  LumiCorProf =  new TProfile("Luminosity Correction", "Luminosity Correction", LumBin, 0., 1.4,0.,2.,"s");
  LumiCor2D =  new TH2F("Luminosity_Correction_2D", "Luminosity_Correction_2D", 100, 0., 1.4,100,0.,2.);
  ...
  if (LumiCorF || RunListF)
    ...
    //
      ...
      //  LumiBin[0] = new TH1F("Lumi", "Lumi", 70, 0., 1.4);
  ...
  for (Int_t i=1 ; i<=NDIR; i++){
	...
	if(LumiCor>0.01 && LumiCor<1.4 && AdditReq )
	  {
		...
		Float_t Sig = 0.007; //From fit
		if (fabs(sqrt(pow(V_x[k],2)+pow(V_y[k],2))-0.094)<5*Sig)   VtxTransPos = true;

	       	//if (fabs(V_x[k]-0.0919)<0.05 && fabs(V_y[k]-0.0189)<0.1)   VtxTransPos = true;
		

		if (NvtxNDOF>=SelNDOF && VtxTransPos)
		  {
	    ...
	    if (Num_Vxt>1 && NHPU < PUBin) PUMulti[Num_Vxt-1]->Fill(MultVtx[NHPU]);
	    ...
	    // Impose a minimum distance between vtx
