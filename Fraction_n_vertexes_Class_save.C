...
#include "Fraction_n_vertexes_Class.h"
#include <TH2.h>
#include <TH2F.h>
#include <TStyle.h>
#include <TCanvas.h>
...
void Fraction_n_vertexes_Class::Loop()
{
  ...
  Dat_all_Lumi = new TH1F("Dat_all_Lumi", "Dat_all_Lumi", LumBin, 0., 0.7);
  Dat_all_Lumi->Sumw2();

  LumiCorProf =  new TProfile("Luminosity Correction", "Luminosity Correction", LumBin, 0., 0.7,0.,2.,"s");
  LumiCor2D =  new TH2F("Luminosity_Correction_2D", "Luminosity_Correction_2D", 100, 0., 0.7,100,0.,2.);
  ...
  if (LumiCorF || RunListF)
    ...
    //
      ...
      //  LumiBin[0] = new TH1F("Lumi", "Lumi", 70, 0., 0.7);
  ...
  for (Int_t i=1 ; i<=NDIR; i++){
	...
	if(LumiCor>0.01 && LumiCor<0.7 && AdditReq )
	  {
